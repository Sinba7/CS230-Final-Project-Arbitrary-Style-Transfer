...
def CORAL(content, style, epsilon=1e-5):
    ...
    def tf_matrix_sqrt(x):
        ...
        s, u, v = tf.svd(x)
        sqrt_x = tf.matmul(tf.matmul(u, tf.sqrt(tf.diag(s))), v)
        return sqrt_x
    ...
    content_flatten = tf.reshape(tf.transpose(content, [0, 3, 1, 2]), 
        [content_shape[0]*content_shape[3], content_shape[1]*content_shape[2]])  # (NC * HW)
    style_flatten = tf.reshape(tf.transpose(style, [0, 3, 1, 2]), 
        [style_shape[0]*style_shape[3], style_shape[1]*style_shape[2]])  # NC * HW
    ...
    content_flatten_norm = (content_flatten - content_mean) / content_std
    style_flatten_norm = (style_flatten - style_mean) / style_std

    content_flatten_cov_eye = tf.matmul(content_flatten_norm, tf.transpose(content_flatten_norm)) + tf.eye(content_shape[0]*content_shape[3])
    style_flatten_cov_eye = tf.matmul(style_flatten_norm, tf.transpose(style_flatten_norm)) + tf.eye(style_shape[0]*style_shape[3])
